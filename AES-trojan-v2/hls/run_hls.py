...
Usage:
  python3 run_hls.py [--xls-root /path/to/xls] [--output-dir ../fpga/rtl]
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Directory for generated RTL output (default: ../fpga/rtl)",
    )
    parser.add_argument(
        "--top-func",
    ...
    src_dir = args.src_dir or os.path.join(script_dir, "src")
    output_dir = args.output_dir or os.path.join(project_root, "fpga", "rtl")
    os.makedirs(output_dir, exist_ok=True)

    # Also keep intermediate IR artifacts in hls/output/
    ir_output_dir = os.path.join(script_dir, "output")
    ...
    # Output file paths (IR artifacts go to hls/output/, final RTL goes to fpga/rtl/)
    ir_file = os.path.join(ir_output_dir, "aes_128.ir")
    opt_ir_file = os.path.join(ir_output_dir, "aes_128.opt.ir")
    verilog_file = os.path.join(output_dir, "aes_128.sv")
    schedule_file = os.path.join(ir_output_dir, "aes_128.schedule.textproto")
    block_file = os.path.join(ir_output_dir, "aes_128.block.textproto")
    signature_file = os.path.join(ir_output_dir, "aes_128.sig.textproto")

    print(f"[HLS] XLS root:    {xls_root}")
    print(f"[HLS] Source:      {src_file}")
    print(f"[HLS] Output dir:  {output_dir}")
    print(f"[HLS] IR dir:      {ir_output_dir}")
