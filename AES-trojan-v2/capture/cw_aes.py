...
Usage:
  ...
  # Save to specific results directory
  python3 cw_aes.py encrypt --random --num-traces 10 --results-dir ../results/experiment1
"""
...
def resolve_results_dir(args):
    """Resolve the results directory from CLI args."""
    if args.results_dir:
        return os.path.abspath(args.results_dir)
    # Default: results/ under project root
...
def save_results(args, operation, plaintexts, ciphertexts_hw, ciphertexts_sw, traces, mismatches):
    ...
    # Default filenames
    mat_file = args.output_mat or os.path.join(results_dir, f"aes_{operation}_{timestamp}.mat")
    json_file = args.output_json or os.path.join(results_dir, f"aes_{operation}_{timestamp}.json")

    # If output files are relative, place them in results_dir
    if args.output_mat and not os.path.isabs(args.output_mat):
        mat_file = os.path.join(results_dir, args.output_mat)
    if args.output_json and not os.path.isabs(args.output_json):
        json_file = os.path.join(results_dir, args.output_json)

    # Ensure parent directories exist
    os.makedirs(os.path.dirname(os.path.abspath(mat_file)), exist_ok=True)
...
Examples:
  ...
  %(prog)s encrypt --random --num-traces 1000 --results-dir ../results/exp1
  %(prog)s decrypt --key 2b7e151628aed2a6abf7158809cf4f3c --data 3ad77bb40d7a3660a89ecaf32466ef97
    ...
    parser.add_argument(
        "--results-dir",
        default=None,
        help="Directory to save results (default: ../results/). Created if it doesn't exist.",
    )
    parser.add_argument(
        "--output-mat",
        default=None,
        help="Filename for .mat output (placed in results-dir if relative)",
    )
    parser.add_argument(
        "--output-json",
        default=None,
        help="Filename for .json output (placed in results-dir if relative)",
    )
