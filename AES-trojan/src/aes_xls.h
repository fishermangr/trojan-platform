// AES-128 ECB Encryption for Google XLS (xlscc)
// Adapted from tiny-AES-C (https://github.com/kokke/tiny-AES-C)
// This version is rewritten for XLS HLS synthesis compatibility.

#ifndef AES_XLS_H_
#define AES_XLS_H_

#include "/xls_builtin.h"  // NOLINT
#include "xls_int.h"       // NOLINT

template <int Width, bool Signed = true>
using ac_int = XlsInt<Width, Signed>;

using u8 = ac_int<8, false>;
using u32 = ac_int<32, false>;

// AES-128 parameters
static const int AES_BLOCKLEN = 16;
static const int AES_KEYLEN = 16;
static const int AES_keyExpSize = 176;
static const int Nb = 4;
static const int Nk = 4;
static const int Nr = 10;

// AES S-box lookup table
inline u8 sbox_lookup(u8 idx) {
  static const u8 sbox[256] = {
    
  };
  return sbox[idx];
}

// Round constants
inline u8 rcon_lookup(int idx) {
  static const u8 rcon[11] = {
    0x8d, 0x01, 0x02, 0x04, 0x08,
    0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
  };
  return rcon[idx];
}

// xtime: multiply by 2 in GF(2^8)
inline u8 xtime(u8 x) {
  u8 shifted = x << 1;
  u8 mask = ((x >> 7) & u8(1)) * u8(0x1b);
  return shifted ^ mask;
}

// I/O structure: 128-bit AES block as 16 bytes
struct AesBlock {
  u8 data[16];
};

#endif  // AES_XLS_H_
